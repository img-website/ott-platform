    ...
    return (
        <div className="flex flex-col py-8 overflow-hidden -mr-5 grow">
            <div className='flex flex-col overflow-auto h-full pr-5'>
                {sideLinks?.map(link => (
                    <Link href={link?.href} key={link?.href} className={`flex items-center sm:gap-3 gap-2 py-3 px-4 rounded-lg text-gray-300 md:hover:text-white md:hover:bg-gray-900 duration-300 ${pathname == link.href ? '!text-white !bg-indigo-500' : ''}`}>
